...
int currentPage = 1, cur = 0, pageSize = 10, totalLinesAdded = 0, totalPageCount = 0, totalLineCount = 0, userPositionInput1 = 0, userPositionInput2 = 0, emptyFile = 0;
...
int main()
{
	...
	while (!anyFileOpened)	// open file
	{
		...
		// Initially Create a new file with first line automatically there
		// because if we don't do it then that file will be consider as bad and can't be open by our program
		else if (command == "create") 
		{ 
			nameOfFile = myFunctions.SplitStringByDelimiterToArray(nameOfFile, ".")[0] + ".txt";
			ofstream newFile;
			newFile.open(nameOfFile);
	...
	while (true) {
		cout << msg.autosave;
		getline(cin, autosaving);
		string* ASave = myFunctions.SplitStringBySpaceToArray(autosaving);
		autosaving = ASave[0];
		autosaving = myFunctions.ToLowerCase(autosaving);
		if (autosaving == "yes") {
			autosave = 1; // 1 means true value for autosaving
			break;
		}
		else if (autosaving == "no") {
			autosave = 0; // 0 means false value for autosaving
			break;
		}
		else {
			cout << msg.validsave;
	...
	while (true)	// operations on file opened
	{
		...
		{
			while (true) // Getting the answer from user whether he/she wants to rename his/her file
			{
				string rename;
				cout << msg.rename;
				getline(cin, rename);
				string* renaming = myFunctions.SplitStringBySpaceToArray(rename);
				rename = renaming[0];
				rename = myFunctions.ToLowerCase(rename);
				
				// If user wants to rename his/her file then its clear that it must be saved after that
				// Getting name for the renaming from the user restricting to alphabets only
				if (rename == "yes") 
				{ 
					cout << msg.renameName;
					getline(cin, userNewFileNameToSave);
					string* newName = myFunctions.SplitStringBySpaceToArray(userNewFileNameToSave);
					userNewFileNameToSave = newName[0];

					userNewFileNameToSave = myFunctions.ToLowerCase(userNewFileNameToSave);
					if(isalphabet(userNewFileNameToSave)){}
					else
					{
						while (!isalphabet(userNewFileNameToSave)) {
							cout << msg.validName;
							getline(cin, userNewFileNameToSave);
							newName = myFunctions.SplitStringBySpaceToArray(userNewFileNameToSave);
							userNewFileNameToSave = newName[0];
							userNewFileNameToSave = myFunctions.ToLowerCase(userNewFileNameToSave);
						}
					}
					userNewFileNameToSave = userNewFileNameToSave + ".txt";

					mainList.save(userNewFileNameToSave);
					cout << userNewFileNameToSave << " created." << endl;
					break;
				}
				else if (rename == "no") {
					if (autosave == 1) {
						// Autosave function before exiting to protect the data 
						userNewFileNameToSave = splittedCommand[1];
